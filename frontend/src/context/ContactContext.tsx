...
import { User, ContactData, ContactResponse, AllContactResponse } from '../types/contact';
...
interface ContactContextType {
    user: User | null;
    isLoading: boolean;
    createContact: (data: ContactData) => Promise<{ data: User | null, error: string | null }>;
...
export const ContactProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const getContactByEmail = async (email: string) => {
        try {
            setIsLoading(true);
            const res = await fetch(`http://localhost:4000/api/contact/contactsByEmail/${email}`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                    // Add authorization token if needed
    ...
    // const getAllContacts = async (page: number, pageSize: number): Promise<AllContactResponse> => {
    //     try {
    //         setIsLoading(true);
    //         const res = await fetch(`http://localhost:4000/api/contact/contacts?page=${page}&pageSize=${pageSize}`, {
    //             method: "GET",
    //             headers: {
    //                 "Content-Type": "application/json",
    //                 // Add authorization token if needed
    ...
    //         }
    //         const response: AllContactResponse = await res.json();
    ...
    const getAllContacts = useCallback(async (params: {
        ...
        try {
            const res = await fetch(`http://localhost:4000/api/contact/contacts?page=${params.page}&pageSize=${params.pageSize}`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                    // Add authorization token if needed
    ...
    const searchContacts = async (query: string) => {
        try {
            setIsLoading(true);
            const res = await fetch(`http://localhost:4000/api/contact/contacts/search?q=${encodeURIComponent(query)}`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                    // Add authorization token if needed
            ...
            const contacts: User[] = await res.json();
            return contacts;
        } catch (error) {
...
export const useContact = () => {
    const context = useContext(ContactContext);
    if (!context) throw new Error('useContact must be used within an ContactProvider');
    return context;
};